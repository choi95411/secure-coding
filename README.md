...
| `REPORTS_PER_HOUR` | 사용자별 시간당 신고 상한 | `10` |
| `MAX_POINT_TRANSACTION` | 1회 송금·조정 포인트 상한 | `1000000000` |
| `MAX_WALLET_BALANCE` | 사용자 지갑 잔액 상한 | `9000000000000000000` |
...
Django ORM 바인딩, CSRF, 세션 회전, HttpOnly/SameSite 쿠키, CSP·Permissions-Policy·SRI, autoescape와 DOM `textContent`, UUID 업로드 경로와 확장자·MIME·크기·이미지 내용 검증을 사용합니다. 송금은 업무 금액 상한, 단일 DB 트랜잭션·행 잠금·고유 멱등 키·QuerySet 우회까지 막는 불변 원장을 사용합니다. 로그인 실패 키는 username/IP 원문 대신 HMAC을 저장하고, 신고는 활성 사용자·시간당 상한을 검사하며, WebSocket은 Origin·세션·활성 상태·대화 참여자를 연결 및 이벤트마다 검사합니다. Django Admin의 직접 상태 변경은 차단하고 모든 제재는 사유와 전후 값이 기록되는 전용 흐름을 사용합니다.
